...
def load_data():
    ...
    df = df[['Player','League','Main Position',
    ...
    df.columns = ['Player','League','Main Position',
...
with tab2:
    ...
    text_colors = ["#FFFFFF"] * len(params)   # White text

    # Instantiate Pizza
    baker = PyPizza(
        params=params,
        background_color="#0A2D57",     # Navy blue background
        straight_line_color="#FFFFFF",  # White lines
        straight_line_lw=1,
        last_circle_lw=0,
        other_circle_lw=0,
        inner_circle_size=5
    ...
    fig, ax = baker.make_pizza(
        ...
        kwargs_values=dict(
            ...
            bbox=dict(edgecolor="#FFFFFF", facecolor="#0A2D57",
    ...
    # --- Add Logo ---
    logo = mpimg.imread("Capture.png")  # path to uploaded logo
    add_image(logo, fig, left=0.82, bottom=0.02, width=0.15, height=0.08)

    # --- Adjustable Width (set here manually) ---
    plot_width = 80   # 👈 change this number to resize the plot
